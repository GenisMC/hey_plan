// Backgrounds
const int backgroundColor = 0xffb6dbde;
const int accentColor = 0xffeb7575;
const int darkerBackgroundAccent = 0xff9bcbcf;
const int darkestBackroundAccent = 0xff6d9fa3;
const int lighterBackgroundAccent = 0xffcce4e6;
const int darkerAccentColor = 0xff70abaf;
// Borders
const int inputBorderColor = 0xffffffff;
const int inputBorderColorFocused = 0xff000000;
const double inputBorderWidth = 2;
// Buttons
const int textButtonColor = 0xff785201;
// Text
const int textColor = 0xff000000;
// Misc
const int cursorColor = 0xff000000;
const double inputFontSize = 17;
const double defaultFontSize = 18;
// Padding
const int padding = 16;
